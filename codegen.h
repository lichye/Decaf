...
// def: Control-Flow Graph: a graph of basic blocks in a function
class CFG {
    List<Instruction*> *code;
    // def record the index of each label
    Hashtable<int*> *labels;
    // * we use Instruction's printed as key
    Hashtable<List<Instruction*>*> *successors;
    Hashtable<List<Location*>*> *gen;
    Hashtable<List<Location*>*> *kill;
    Hashtable<List<Location*>*> *in;
    Hashtable<List<Location*>*> *out;
    // def the graph to run Chaitin's algorithm on
    List<List<Location*>*> *graphMap;
    List<Location*>* graphKey;

    List<Location*> *variables;
    List<Location*> *chaitinVariables;
protected:
    static bool IsIn(List<Register> *list,Register reg);
    static bool IsIn(List<Location*> *list,Location* loc);
    static bool IsSame(List<Location*>* a,List<Location*>* b);
    // return how many nodes are still unknown
    int NumUnknown(List<Location*> *list);
    // if we can't find any available register, return spill
    static Register FindAvailableRegister(List<Register> *list);
    // fill Kill[TAC], Gen[TAC] and initialize In[TAC], Out[TAC}
    void GetSets();
    // fill successors for each instruction
    void GetSuccessors();
  public:
    CFG();
    // must be called before AppendInstruction()
    bool GetCurrentIndex() { return code->NumElements(); }
    // return whether all labels are already pushed to code
    bool AllLabels() const;
    // if we need the label, index should set to -1
    void AppendLabel(const char* label, int index = -1);
    void AppendInstruction(Instruction* instruction);
    // run live variable analysis algorithm
    void LiveAnalysis();
    // build the interference graph
    void InterfereGraph();
    // color, get register and spill node
    void Chaitin();
    // emit all optimized instructions, will destroy the graph
    void Emit(Mips *mips);
};
...
class CodeGenerator {
  ...
  public:
    ...
    char *NewLabel();
     ...
     //when end a function we should reset the offset
    int GetLocalOffset() { return offset; }
    void SetLocalOffset(int offset_in) { this->offset = offset_in; }
    void resetLocalOffset() { offset = OffsetToFirstLocal; }
    Location* GenLocalVar(char*name,int size) {
         Location* ret = new Location(fpRelative,offset,name);
         offset -= size;
         return ret;
    }
    ...
    // * Refer to tac.h for these instructions
    ...
    void DoFinalCodeGen();
        ...
        // Generates the TAC instructions for NewArrayExpr,
        // check err_arr_bad_size
    Location *GenNewArray(Location *numElements);

    void GenHaltWithMessage(const char *msg);

        // Generates the TAC instructions for Call,
        // check err_arr_out_of_bounds
